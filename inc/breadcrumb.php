		...
		// If not viewing the front page.
			...
			if (is_singular()) {
				...
				if (get_post_type() == 'unita_organizzativa') {
					...
					if ($terms) {
						...
						foreach ($terms as $term) {
							switch ($term->slug) {
								case 'ufficio':
									$page = get_page_by_path('amministrazione/uffici');
									$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_permalink($page->ID)), $page->post_title);
									break;
								case 'ente':
								case 'fondazione':
									$page = get_page_by_path('amministrazione/enti-e-fondazioni');
									$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_permalink($page->ID)), $page->post_title);
									break;
								case 'area':
									$page = get_page_by_path('amministrazione/aree-amministrative');
									$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_permalink($page->ID)), $page->post_title);
									break;
								...
								case 'giunta-comunale':
									$page = get_page_by_path('amministrazione/organi-di-governo');
									$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_permalink($page->ID)), $page->post_title);
									break;
							}
						}
				...
				if (get_post_type() == 'persona_pubblica') {
					$this->items[] = "<a href='" . home_url("amministrazione") . "'>" . __("Amministrazione", "design_comuni_italia") . "</a>";
					$incarichi = dci_get_meta("incarichi", $prefix, get_post()->ID);
					if ($incarichi) {
						$incarico_ = get_post($incarichi[0]);
						$lista_incarichi[] = $incarico_->post_title;
						$a = get_the_terms($incarico_, 'tipi_incarico');
						$tipo_incarico = $a[0]->name;
						switch ($tipo_incarico) {
							case 'amministrativo':
								$page = get_page_by_path('amministrazione/personale-amministrativo');
								$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_permalink($page->ID)), $page->post_title);
								break;
							case 'politico':
								$page = get_page_by_path('amministrazione/politici');
								$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_permalink($page->ID)), $page->post_title);
								break;
						}
					}
